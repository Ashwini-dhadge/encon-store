...
class VendorModel extends CI_Model
{
    ...
    {
        $this->db->select('vmd.*,vm.*,vm.id as vendor_id,c.name as city_name, vm.vendor_code as vendor_code');
    ...
    // {
    //     $this->db->select('mv.*,vmd.address_details, mv.vendor_code as vendor_code');
    //     $this->db->from('tbl_vendor_master mv');
    //     $this->db->join('tbl_vendor_master_details vmd', 'vmd.vendor_id=mv.id');
    //     if ($searchTerm) {
    //         $searchCondition = "(
    //                 mv.account_name, mv.vendor_code like '%$searchTerm%'
    //              )";
    //         $this->db->where($searchCondition);
    //     }
    ...
    public function getVendorName($searchTerm = '')
    {
        $this->db->select('mv.*, vmd.address_details, mv.vendor_code');
        $this->db->from('tbl_vendor_master mv');
        $this->db->join('tbl_vendor_master_details vmd', 'vmd.vendor_id = mv.id');

        if ($searchTerm) {
            $this->db->group_start();
            $this->db->like('mv.account_name', $searchTerm);
            $this->db->or_like('mv.vendor_code', $searchTerm);
            $this->db->group_end();
        }
