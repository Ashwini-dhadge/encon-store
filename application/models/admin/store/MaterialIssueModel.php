<?php
   class MaterialIssueModel extends CI_Model
   {
       ...
       protected $dt_Column = array(
        ...
        public function getMaterialIssueData($searchVal='',$sortColIndex='0',$sortBy='desc',$limit='0', $offset='0',$id='',$where="",$item_id='')
         {
            if(isset($item_id) && !empty($item_id)){
                 $subquery = $this->db->select('mii.issue_id')
                     ->from('tbl_material_issue_items_details mii')
                     ->where('mii.item_id', $item_id)
                     ->where('mii.deleted_by', NULL)
                     ->where('mii.issue_id = mi.id', NULL, false)
                     ->get_compiled_select();
