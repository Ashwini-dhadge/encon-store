...
class GRNModel extends CI_Model
{
    public function getGRNNumber()
    {
        ...
        if (isset($result['grn_order_sequence'])) {
            if($company_id==7 && $site_id==21){
                //missing sequence  261 to 456 that we want add after 456 start sequence 473
                // if($result['grn_order_sequence']==475){
                //     $grn_number = 261;
                // }else if($result['grn_order_sequence']==456){
                //     $grn_number = 476;
                // }else{
        ...
        $grn_no = userId('site_inital') . "/".$prefix_year. '/' .$grn_number;
        $number=array();
        $number['grn_order_sequence']=$grn_number;
        $number['grn_order']=$grn_no;
        return $number;
    }
    ...
    public function getPOPendingCount($where)
    {
        ...
        $this->db->join('tbl_items i', 'i.id=pi.item_id');
        $this->db->join('tbl_po p', 'p.id=pi.po_id');
        $this->db->join('tbl_grn_items_details gi', 'gi.po_item_id=pi.id and gi.po_id=pi.po_id and pi.item_id = gi.item_id and gi.deleted_by is NULL', 'LEFT');
    ...
    public function getGRNData($searchVal = '', $sortColIndex = '0', $sortBy = 'desc', $limit = '0', $offset = '0', $id = '', $where = '', $item_id = '',$item_group_id='')
    {
        if (isset($item_id) && !empty($item_id) || isset($item_group_id) && !empty($item_group_id)) {
                $this->db->select('pi.grn_id');
                ...
                $this->db->where('pi.deleted_by', null);
                $this->db->where('pi.grn_id = p.id', null, false);
    ...
    public function getGrnItemDetailsData($id = '')
    {
        ...
        $this->db->join('tbl_po_items_details pid', 'pid.po_id=gid.po_id and pid.item_id=gid.item_id and pid.id=gid.po_item_id ', 'left');
