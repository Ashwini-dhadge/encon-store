        ...
        public function getReceviedMaterialData($searchVal='',$sortColIndex='0',$sortBy='desc',$limit='0', $offset='0',$id='',$where='',$item_id='',$item_group_id='')
        {
         ...
         public function getMaterialIssuePendingCount($where_in_array,$where=''){
            //  print_r($where);
                ...
                $this->db->join('tbl_recevied_material_issue_items_details gi','gi.issue_id=pi.issue_id and pi.item_id = gi.item_id and `pi`.`batch_no` = `gi`.`batch_no` and `pi`.`expired_date` = `gi`.`expired_date`  and gi.deleted_by IS NULL and pi.issue_qty=gi.received_qty','LEFT'); 
