<?php
   class GoodsItemOpeningModel extends CI_Model
   {
       ...
       protected $dt_Column = array(
           ...
           public function getOpeningStockData($searchVal='',$sortColIndex='0',$sortBy='desc',$limit='0', $offset='0',$id='',$where="",$item_id='')
         {
             if(isset($item_id) && !empty($item_id)){
                 $subquery = $this->db->select('pi.opening_id')
                     ->from('tbl_items_opening_stock_details pi')
                     ->where('pi.item_id', $item_id)
                     ->where('pi.deleted_by', NULL)
                     ->where('pi.opening_id = mi.id', NULL, false)
                     ->get_compiled_select();
