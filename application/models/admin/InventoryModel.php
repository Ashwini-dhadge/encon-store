   ...
   class InventoryModel extends CI_Model
   {
       ...
       protected $dt_Column = array(
        ...
        public function getInventoryDetailsData($searchVal='',$sortColIndex='0',$sortBy='desc',$limit='0', $offset='0',$id='',$where="",$where1="",$inv_item_unit_id="")
         ...
         public function getSUMQtyInventoryDetailsData($searchVal='',$sortColIndex='0',$sortBy='desc',$limit='0', $offset='0',$id='',$where="",$where1="",$inv_item_unit_id="")
         {
           
           $this->db->select(' SUM(CASE WHEN `indt`.`action` = 1 THEN `indt`.`qty` ELSE 0 END) -  SUM(CASE WHEN `indt`.`action` = 2 THEN `indt`.`qty` ELSE 0 END) AS final_qty');
           $this->db->join('tbl_items i','i.id=indt.item_id');
           $this->db->join('tbl_items_units iu','iu.id=indt.item_unit_id');
