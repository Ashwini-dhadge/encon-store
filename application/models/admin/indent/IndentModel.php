...
class IndentModel extends CI_Model
{
    ...
    public function getindentData($searchVal = '', $sortColIndex = '0', $sortBy = 'desc', $limit = '0', $offset = '0', $id = '', $where = "")
    {
        ...
        if (!empty($searchVal)) {
            $this->db->group_start();
            $this->db->like('ti.indent_no', $searchVal);
             $this->db->or_like([
                'c.company_name' => $searchVal,
                'p.plant_narration' => $searchVal,
                'ti.indent_number' => $searchVal
            ]);
            $this->db->group_end();
        }
    ...
    {
        $this->db->select('ti.*, min.db_table_name, min.indent_name, s.site_name, min.db_qty_field_name,min.field_count,in.indent_number,c.left_image,c.name as company_name ,c.address as company_address,c.email as company_email,s.site_address,c.right_image,u.first_name,u.last_name,c.signature_image ');
        $this->db->join('tbl_indent in', 'in.id = ti.indent_id');
        $this->db->join('tbl_site s', 's.id = ti.plant_id');
    ...
    public function getHubSize($site_id = '')
    {
        $this->db->select('hub_size');
        $this->db->from('tbl_site');
        $this->db->where('id', $site_id);
        $result = $this->db->get()->row_array();
        return $result['hub_size'];
    }

    public function getClampSize(){
        $this->db->select('tic.clamp_size, tics.length as clamp_length');
        $this->db->from('tbl_indent_frp_clamp tic');
        $this->db->where('tic.deleted_by', NULL);
        $this->db->join('tbl_frp_clamp_specifications tics', 'tic.indent_id = tics.indent_id');
