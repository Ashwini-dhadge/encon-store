...
class BoxPO extends CI_Controller
{
    ...
    public function get_boxpo_list()
    {
        $filters = $this->input->post();
        $list = $this->BoxPOModel->getBoxPOList($filters);

        $data = [];
        foreach ($list as $row) {

            $status = ($row->boxpo_status == 1)
                ? '<span class="badge badge-success">Approved</span>'
                : '<span class="badge badge-warning">Pending</span>';

            $pdf = '';
            $pdfFile = 'BOX_PO_' . $row->id . '.pdf';

            if (file_exists(FCPATH . 'uploads/boxpo/' . $pdfFile)) {
                $pdf = '
                <a href="' . base_url('uploads/boxpo/' . $pdfFile) . '" 
                target="_blank" 
            ...
            $actions = '
                        <a href="' . base_url('admin/BoxPO/add/' . $row->id) . '" 
                        class="btn btn-primary waves-effect waves-light btn-sm" title="View">
                        <i class="fa fa-eye"></i>
                        </a>
                        <a href="' . base_url('admin/BoxPO/add/' . $row->id) . '" 
                        class="btn btn-primary waves-effect waves-light btn-sm" title="Edit">
            ...
            $data[] = [
                $row->boxpo_order_no,
                date('d-m-Y', strtotime($row->boxpo_date)),
                $row->vendor_name,
                $row->company_name,
                $row->site_name,
                number_format($row->total_sq_inch, 0),
                number_format($row->cu_ft, 2),
    ...
    public function add($id = '', $type = 0)
    {
        ...
        $number = $this->BoxPOModel->getPONumber();
        ...
        if (!empty($id)) {
            ...
            $data['po_info'] = $this->db
                ->get_where('tbl_box_po', ['id' => $id])
                ->row_array();

            $data['dimensions'] = $this->db
                ->get_where('tbl_box_po_dimensions', ['box_po_id' => $id])
                ->result_array();
    ...
    public function add_boxpo()
    {
        ...
        $vendor = $this->CommonModel->getData(
            'tbl_vendor_master',
            ['id' => $po['vendor_id']],
            '',
            '',
        ...
        $dimensions = $this->CommonModel->getData(
            'tbl_box_po_dimensions',
            ['box_po_id' => $boxPoId]
        );
    ...
    public function listBoxPo()
    {
        ...
        $sortCol = $data['order'][0]['column'] ?? 0;
        $sortDir = $data['order'][0]['dir'] ?? 'DESC';
        ...
        if ($count > 0) {
            ...
            foreach ($result as $key => $value) {
                ...
                array_push(
                    $row,
                    '<a href="' . base_url('admin/BoxPO/add/' . $value['id']) . '" title="View">'
