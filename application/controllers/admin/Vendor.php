...
class Vendor extends CI_Controller
{
    ...
    public function add_vendor($_id = '')
    {
        ...
        if (empty($_id)) {
            ...
            $this->db->limit(1);
            $lastVendor = $this->db->get()->row_array();
            if (!empty($lastVendor)) {
                $num = (int) str_replace('VND-', '', $lastVendor['vendor_code']);
                $num++;
                $vendor_code = 'VND-' . str_pad($num, 4, '0', STR_PAD_LEFT);
            } else {
    ...
    public function submit_vendor_data($_id = '')
    {
        ...
        if ($post) {
            ...
            $vendor = array(
                ...
                'vendor_code' => isset($post['vendor_code']) ? $post['vendor_code'] : '',
    ...
    public function listVendorName($value = '')
    {
        ...
        // Empty option
        $json[] = ['id' => '', 'text' => 'Select Vendor'];

        foreach ($vendor_name as $key => $value) {
            $json[] = [
                'id' => $value['id'],
                'text' => $value['account_name'] . ' (' . $value['vendor_code'] . ')',
                'address' => $value['address_details']
            ];
        }
