...
//                         die;
       ...
       $attachement = array();
        $post = $this->input->post();
            ...
            if ($post){
                        ...
                        if(isset($post['grn_order_sequence'])){
                             $grn_order_sequence =$post['grn_order_sequence'];
                        }else{
                        ...
                        $insert_tax_data = array(
                                ...
                                'new_tax_rate' => (isset($post['new_tax_amount'])) ? $post['new_tax_amount'] :NULL,
                                    ...
                                    //   print_r($item);
                                       ...
                                       if(isset($item['item_weight']) && !empty($item['item_weight'])){
                                        ...
                                        $insert_item_data = array(
                                                ...
                                                'item_weight_unit'=>(isset($item['weight_unit']))?$item['weight_unit']:NULL,
                                        ...
                                        if (isset($item['grn_item_id']) && !empty($item['grn_item_id'])) {
                                                ...
                                                $insert_item_data['updated_at'] = date('Y-m-d H:i:s');
     ...
     public function listPo()
    {
        ...
        $count = count($this->POModel->getPOData($searchVal,0,0,0,0,0,$where,$item_id,$item_group_id));
        
        if($count){
            $result = $this->POModel->getPOData($searchVal, $sortColIndex, $sortBy, $limit, $offset,0,$where,$item_id,$item_group_id);
            // echo $this->db->last_query();die;
                foreach ($result as $key => $value) {
                    ...
                    $action='<a href="'.base_url().'admin/store/GoodsReceiptNote/add_Goods_Receipt_Note/2/'.$value['id'] .'" title="po_pdf" class="btn btn-primary waves-effect waves-light btn-sm editModal"data-toggle="tooltip" style="font-size:13px;background:#F0F0F0;color: gray;">Create GRN</a>'; 
                     if(isset($value['po_invoice_pdf_name'])){
                             $action.='<a href="'.base_url().PO_PDF_PATH.$value['po_invoice_pdf_name'] .'" title="po_pdf" target="_blank" class="btn btn-primary waves-effect waves-light btn-sm editModal"data-toggle="tooltip" style="font-size:13px;background:#F0F0F0;color: gray;"><i class="fas fa-print" aria-hidden="true"></i></a>';
                        }else{
                            $action.='';
                        }
    ...
    public function listGRN()
    {
        ...
        $count = count($this->GRNModel->getGRNData($searchVal,0,0,0,0,0,$where,$item_id,$item_group_id));
        // echo $this->db->last_query();
        if($count){
            $result = $this->GRNModel->getGRNData($searchVal, $sortColIndex, $sortBy, $limit, $offset,0,$where,$item_id,$item_group_id);
            // echo $this->db->last_query();die;
                foreach ($result as $key => $value) {
                    $flag=$own_flag_access=$issu_count=0;
                    $row = []; 
                  
                   if(!empty($value['po_id'])){
                        $po_data = $this->CommonModel->getData('tbl_po', array('id' => $value['po_id']), 'po_grn_status,po_order_no', '', 'row_array');
                        $po_order_no=(isset($po_data['po_order_no']))?$po_data['po_order_no']:'';
                    ...
                    if($po_order_no){
                        array_push($row,'<a href="'.base_url().'admin/PO/viewPoDetailsData/'. $value['po_id'].'" target="_blank">'.$po_order_no.'</a>');
                    }else{
                        array_push($row,'');
                    }
        ...
        if(!empty($grn_data) && !empty($grn_item_data)){
             ...
             if($flag==1){
                   $response['result']=false;
                    // $response['reasons']=$grn_data['po_id'];
                   $response['reasons']="You can not delete GRN . becuse some item issued ";
                    echo json_encode($response);
                    die;
                      $this->session->set_flashdata('error','You can not delete GRN . becuse some item issued'.$getItemQty);
             }else{
