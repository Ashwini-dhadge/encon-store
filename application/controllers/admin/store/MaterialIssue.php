...
class MaterialIssue extends CI_Controller
{
   ...
   public function list_material_issue()
   {
        ...
        if(!empty($data['item_group_id']) &&  isset($data['item_group_id']) && $data['item_group_id']!="all"){
         ...
         $count = count($this->MaterialIssueModel->getMaterialIssueData($searchVal,0,0,0,0,0,$where,$item_id,$item_group_id));
         if($count){
               $result = $this->MaterialIssueModel->getMaterialIssueData($searchVal, $sortColIndex, $sortBy, $limit, $offset,0,$where,$item_id,$item_group_id);
               ...
               foreach ($result as $key => $value) {
                ...
                $is_delete=1;
                 if($value['issue_type']==1){
                    ...
                    $num_rows_transfer = $this->CommonModel->getData('tbl_material_issue_items_details', array('issue_id' => $value['id'],'deleted_by'=>NULL,'is_transfer_completed'=>1), '', '', 'num_rows');
                    $total_items_transfer = $this->CommonModel->getData('tbl_material_issue_items_details', array('issue_id' => $value['id'],'deleted_by'=>NULL), '', '', 'num_rows');
                    if($num_rows_transfer==$total_items_transfer){
                         $is_delete=0;
                    }else{
                        $is_delete=1;
                    }
                ...
                $confirm = "confirm('Are you sure you want to delete this GRN . Its effect on item Inventory ?')";
                    if((getUserAccessForModule('Store','delete') || ($own_flag_access) || userId('role_id')==SUPERADMIN_ROLE)&&($is_delete==1)):
       ...
       // $po_item_id=$post['po_item_id'];
        ...
        if(!empty($data) && !empty($item_data) ){
                ...
                if($data['issue_type']==2){
                    $destination_company_ids= $this->CommonModel->getData('tbl_site',array('id'=>$data['issue_to_location_site_id']),'id,company_id','','row_array');
                }     
                ...
                foreach($item_data as $key=>$item){
                    ...
                    if($data['issue_type']==2){
                        ...
                        updateQuickInventory($item['item_id'],$item['issue_qty_unit'],$item['issue_qty'],$destination_company_ids['company_id'],$data['issue_to_location_site_id'],userId('financial_year_id') ,INVENTORY_ACTION_ISSUE_MINUS,ISSUE_TYPE,$issue_id,userId(),$item['batch_no'],$item['expired_date'],IS_INVENTORY_REVERSED,$item['id']);
                    }
                ...
                $json_data_login=encode_arr($item_data);
                ...
                $data_action_log_array=array(userId(),userId('role_id'),userId('common_financial_year'),userId('company_id'),userId('site_id'),'delete',$description,5,$issue_id,'','',date('Y-m-d H:i:s'),$json_data_login);;
    ...
    public function add_materialissue($_id='')
    {
            ...
            $post = $this->input->post();
                ...
                $insert_items = array(
                        ...
                        'issue_order_sequence'=> isset($post['issue_order_sequence'])?$post['issue_order_sequence']:0, 
        ...
        if ($post) {
                     if (empty($post['issue_id'])) {
                        ...
                        unset($insert_items['issue_order_sequence']);
                        unset($insert_items['issue_number']);
                    ...
                    if ($insert_id) {
                        ...
                        $delete_item_array=$delete_item_id_array=array();
                        if(isset($post['issue_id']) && !empty($post['issue_id'])){
                            ...
                            $old_item_array=array_column($old_grn_item_data, 'id');
                            $new_item_array=array_column($post['material_items'], 'old_issue_item_id');
                             
                             //array difference
                             $delete_item_id_array= array_diff($old_item_array, $new_item_array);
                             
                        }
                        // echo "<pre>";
                        // print_r($delete_item_id_array);
                        ...
                        {             
                                        ...
                                        $is_inv_add=$is_inv_minius=$update_qty=$is_new_batch='';
                                         if(isset($value1['old_issue_item_id'])){
                                             $get_issue_item_data = $this->CommonModel->getData('tbl_material_issue_items_details', array('issue_id' =>$post['issue_id'],'deleted_by' => NULL ,'id'=>$value1['old_issue_item_id']),'','','row_array');
                                        }else{
                            ...
                            $material_issue = array(
                             ...
                             if(isset($post['issue_id'])  && ! empty($get_issue_item_data)){
                                    ...
                                    $material_issue['updated_by'] = userId();
                                      ...
                                      if(isset($value1['old_issue_item_id'])){
                                                $this->CommonModel->iudAction('tbl_material_issue_items_details',$material_issue,'update',array('id' => $value1['old_issue_item_id'],'issue_id' =>$post['issue_id']));
                                                $sub_item_id['id']=$value1['old_issue_item_id'];
                                        }else{
                            ...
                            if(isset($post['issue_id']) && !empty($delete_item_id_array)){
                                    //update case if grn Item Delete From 
                                    // if(empty($delete_item_id_array)){
                                    ...
                                    //     }
                                    // }else{
                                        if(count($delete_item_id_array) !=0){
                                        
                                            foreach ($delete_item_id_array as $key => $delete_item) {
                                                
                                                $get_item_data=array();
                                                $get_item_data = $this->CommonModel->getData('tbl_material_issue_items_details', array('issue_id' => $post['issue_id'], 'deleted_by' => NULL ,'id'=>$delete_item),'','','row_array');
                                                ...
                                                if(isset($post['issue_id']) && !empty($get_item_data) && isset($get_item_data['issue_qty']) && isset($get_item_data['batch_no'])){
                                                    ...
                                                    $insert_item_data=array();
                                                    $insert_item_data['deleted_at'] = date('Y-m-d H:i:s');
                                                    $insert_item_data['deleted_by'] = userId();
                                                    
                                                    $sub_item_id=$this->CommonModel->getData('tbl_material_issue_items_details', array('id' => $delete_item, 'issue_id' => $post['issue_id']),'id','','row_array');
                                                    $this->CommonModel->iudAction('tbl_material_issue_items_details', $insert_item_data, 'update', array('id' => $delete_item,'issue_id'=>$post['issue_id']));
