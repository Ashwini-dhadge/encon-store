        ...
        if($count){
            ...
            // echo $this->db->last_query();die;
                foreach ($result as $key => $value) {
                    ...
                    // $issue_total_qty= $this->CommonModel->getData('tbl_recevied_material_issue_items_details',array('deleted_by'=>NULL,'issue_id'=>$value['id']),' sum(received_qty) as received_qty','','row_array');
                    // echo $this->db->last_query();die;
            ...
            foreach($data['info_items_data'] as $key=>$info_item){
                ...
                if($info_item['item_group_id']==0){
                  $data['info_items_data'][$key]['item_name'] = $this->CommonModel->getData('tbl_items',array('deleted_by'=>NULL));
                   $data['info_items_data'][$key]['query_item_name'] = $this->db->last_query(); 
                }else{
    ...
    public function list_material_issue_items_data()
   {
    ...
    public function ViewMaterialIssue($material_issue_id='')
    {
	 ...
	 function getTrasfterMaterialInternaly2(){
        ...
        // $financial_year_id = isset($data['financial_year_id'])?$data['financial_year_id']:userId('financial_year_id');
        // $getMateraialIssued = ($this->ReceiveMaterialIssueModel->getMaterialIssueTransferData('',0,0,0,0,0,$where,'',''));
        $getMateraialIssued =$this->CommonModel->getData('tbl_material_issue',array('issue_to_location_site_id'=>39 ,'is_transfer_completed'=>0 ,'deleted_by'=>NULL),'','','');;
        // echo "<pre>";
        // echo count($getMateraialIssued);
        // die;
        // copy material issue table in discard table
        $tr_data=array();
        $tr_data_no_insert=array();
        $total=0;
      
        foreach($getMateraialIssued as $key=>$value){
           
            $discrad_data=$value;
            $discrad_data['material_issue_id']=$discrad_data['id'];
            unset($discrad_data['id']);
            $discrad_data['created_by']=userId();
         
            $material_issue_discard_id=$this->CommonModel->iudAction('tbl_material_issue_discard',$discrad_data , 'insert');
            
            $materialItemDetails=$this->CommonModel->getData('tbl_material_issue_items_details',array('issue_id'=>$value['id'],'is_transfer_completed '=>0,'deleted_by'=>NULL),'','','');
          
            if(!empty($materialItemDetails)){
                foreach($materialItemDetails as $key1=>$value1)
                        {             
                            $item_id=$value1['item_id'];
                            $item_unit_id=$value1['issue_qty_unit'];
                            $qty=$value1['issue_qty'];
                            $batch_no=$value1['batch_no'];
                            $expired_date=$value1['expired_date'];
                            $user_id=userId();
                            $company_id=2;
                            $site_id=39;

                            $data_discard=$value1;
                            $data_discard['material_issue_item_id']=$value1['id'];
                            $data_discard['discard_id']=$material_issue_discard_id; 
                         ...
                         if($item_unit_id && $item_id && $batch_no && $expired_date && $expired_date!='0000-00-00'){
                           ...
                           $row_data['inv_qty'] = isset($dataQty['qty'])?$dataQty['qty']:0;

                            


                           // Get sum of qty for action=1, action=2, and their balance
                           $sql = "SELECT 
                               SUM(CASE WHEN action=1 THEN qty ELSE 0 END) AS sum_action1,
                               SUM(CASE WHEN action=2 THEN qty ELSE 0 END) AS sum_action2,
                               SUM(CASE WHEN action=1 THEN qty ELSE 0 END) - SUM(CASE WHEN action=2 THEN qty ELSE 0 END) AS balance
                               FROM tbl_items_inventory_details
                               WHERE item_id=? AND item_unit_id=? AND company_id=? AND site_id=? AND financial_year_id=? AND batch_no=? AND expired_date=? AND is_reserve_stock=1";
                           ...
                           $dataDetailsQty = $query->row_array();
                           $balance = isset($dataDetailsQty['balance']) ? $dataDetailsQty['balance'] : 0;
                           ...
                           $tr_data[] = $row_data;
                            ...
                            // echo "<pre>";
                            // print_r($inst);die;
                        $this->CommonModel->iudAction('tbl_items_inventory_details', $inst, 'insert');
                         
                         // UPDATE QTY IN ITEM INVENTORY TABLE
                        //  $new_qty=$row_data['inv_qty'] - $balance;
                        $this->CommonModel->iudAction('tbl_items_inventory', array('qty'=>0), 'update', array('id'=>isset($dataQty['id'])?$dataQty['id']:0) );
                        
                        }else{
                            $tr_data_no_insert[] = array('issue_id'=> $value['id'],'message'=>'No pending items to transfer');
                        }
                        }
                    }
                          
        }
        echo "Total items processed: " . $total . "<br>";

        //
          // Display $tr_data_no_insert as an HTML table if it has data
        if (!empty($tr_data_no_insert) && isset($tr_data_no_insert[0]) && is_array($tr_data_no_insert[0])) {
            echo '<br><b>Items not inserted:</b><br>';
            echo '<table border="1" cellpadding="5" cellspacing="0" style="border-collapse:collapse;">';
            echo '<thead><tr>';
            foreach (array_keys($tr_data_no_insert[0]) as $header) {
                echo '<th>' . htmlspecialchars($header ?? '') . '</th>';
            }
            echo '</tr></thead><tbody>';
            foreach ($tr_data_no_insert as $row) {
                echo '<tr>';
                foreach ($row as $cell) {
        ...
        // Display $tr_data as an HTML table
        if (!empty($tr_data) && isset($tr_data[0]) && is_array($tr_data[0])) {
            echo '<table border="1" cellpadding="5" cellspacing="0" style="border-collapse:collapse;">';
            echo '<thead><tr>';
            foreach (array_keys($tr_data[0]) as $header) {
                echo '<th>' . htmlspecialchars($header ?? '') . '</th>';
            }
            echo '</tr></thead><tbody>';
            foreach ($tr_data as $row) {
                // Only show row if inv_qty and balance are not the same
                $inv_qty = isset($row['inv_qty']) ? (float)$row['inv_qty'] : 0;
