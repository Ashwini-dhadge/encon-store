...
class GoodsItemOpening extends CI_Controller
{
       ...
       // $po_item_id=$post['po_item_id'];
        ...
        if(!empty($data) && !empty($item_data)){
             foreach($item_data as $key=>$item){
                $getItemQty=getQuickInventoryAmount($item['item_id'],$item['opening_unit_id'],$data['company_id'],$data['site_id'],$data['financial_year_id'],$item['batch_no'],$item['expired_date'],0);
                if($item['opening_qty'] > $getItemQty){
                    $flag=1;
                }
             }
         ...
         //   echo $flag;die;
             if($flag==1){
                   ...
                   $response['reasons']="You can not delete Opening STOCK . becuse some item issued ";
    ...
    public function list_opening_stock()
    {
        ...
        //  echo $this->db->last_query();die;
         if($count){
               $result = $this->GoodsItemOpeningModel->getOpeningStockData($searchVal, $sortColIndex, $sortBy, $limit, $offset,0,$where,$item_id,$item_group_id);
