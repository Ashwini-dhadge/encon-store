...
class PO extends CI_Controller
{
        ...
        public function getItemUnitsData()
        {
                ...
                foreach($items_units as $key=>$value){
                 ...
                 foreach($items_batchs_no as $key=>$value){
                    $amt=getQuickInventoryAmount($value['item_id'],$value['item_unit_id'],userId('company_id'),userId('site_id'),userId('financial_year_id'),$value['batch_no']);
       ...
       // print_r($cites);die;
           ...
           public function getUnitBatchWiseData()
        {
                ...
                if (isset($post['item_id'])) {
                  ...
                  $items_batchs_no = $this->POModel->getItemUnitBatchList(array('i.item_id'=>$post['item_id'],'i.item_unit_id'=>$post['item_unit_id'],'i.company_id'=>userId('company_id'),'i.site_id'=>userId('site_id'),'i.financial_year_id'=>userId('financial_year_id')));
                // echo $this->db->last_query();
                 foreach($items_batchs_no as $key=>$value){
                    ...
                    $rate= $this->POModel->getItemRateBatchList(array('gi.batch_no'=>$value['batch_no'],'expired_date'=>$value['expired_date'],'item_id'=>$value['item_id'],'item_unit_id'=>$value['item_unit_id'],'grn.company_id'=>userId('company_id'),'grn.site_id'=>userId('site_id'),'grn.financial_year_id'=>userId('financial_year_id')));
                    //   echo $this->db->last_query();
       ...
       // print_r($cites);die;
        ...
        {
                $post = $this->input->post();
                   // echo "<pre>";print_r($_POST['removedFiles']);print_r();die;
                //         if (!empty($_FILES['files']['name'][0])) {
                //                 $uploadStatus = myUpload(PO_ATTACH, 'files', true);
                ...
                //         }
                //   echo "<pre>"; print_r($attachement); die;
                if ($post) {
                        ...
                        if(isset($post['po_order_no'])){
                           ...
                           // $random = mt_rand(100, 999);
                            //$po_order_no=$post['po_order_no']."-".$random;
                            ...
                            if($is_add==1){
                                ...
                                $po_order_sequence=$po_order_sequence+1;
                                $po_order_no=userId('site_inital')."-".date('Y')."-".$po_order_sequence;
                            }
                            
                        }else{
                            $number=$this->POModel->getPONumber();
                           // $po_order_no = $number['po_no'];
                            $po_order_sequence = $number['po_order_sequence']+1;
                            $po_order_no=userId('site_inital')."-".date('Y')."-".$po_order_sequence;
    ...
    public function sendMailWithattchement(){
       ...
       if(empty($cc_array)){
        $cc_array=array();
          //$cc_array=array('ramkrushna@encongroup.in','enconrksharma@gmail.com','ritikshukla@encongroup.in');
       }else{
