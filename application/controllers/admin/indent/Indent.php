...
class Indent extends CI_Controller
{
  ...
  public function listindent()
  {
    ...
    if ($count) {
      ...
      foreach ($result as $key => $value) {
        ...
        $action .= '<a href="' . base_url() . 'admin/indent/Indent/generate_pdf/' . $value['id'] . '" target="_blank" title="print indent" class="btn btn-primary waves-effect waves-light btn-sm editModal"data-toggle="tooltip" style="font-size:13px;background:#F0F0F0;color: gray;"><i class="fas fa-print" aria-hidden="true"></i></a>';
  ...
  public function generate_pdf($id = '', $indent_id = '')
  {
    ...
    foreach ($groupedIndentData as $key => $record) {
      ...
      //remove this site wise no want
      ///$groupedIndentData[$key]['indent_details'] =  $indentData;
    }
    $groupedIndentData= $this->IndentModel->getIndentViewData(array('ti.indent_id' => $id), 0);
