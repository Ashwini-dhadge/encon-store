...
function listbladeindent(data = "") {
  bladeindent = $("#blade_indent").DataTable({
    ...
    order: [[1, "desc"]],
    ajax: {
      url: base_url + "admin/production/Indent/list_header_pipe",
      type: "POST",
      dataSrc: "data",
      data: data,
    },
...
function listReceivedOrders(data = "") {
  receivedOrderTable = $("#receivedOrders").DataTable({
    ...
    order: [[1, "desc"]],
    ajax: {
      url: base_url + "admin/production/Indent/list_frp_header_pipe_received_orders",
      type: "POST",
      dataSrc: "data",
      data: data,
    },
    ...
    columns: [
      ...
      { width: "6%", title: "Size Cutting" },
      { width: "6%", title: "Flange Making" },
      { width: "6%", title: "Pipe & Coupler Joining" },
      { width: "6%", title: "Finishing" },
...
$(document).on('click', '#export_selected_pdf', function () {
  ...
  let url =
    base_url +
    'admin/production/Indent/export_selected_frphp_pdf?ids=' +
    selected.join(',');
...
$(document).on('click', '#export_selected_excel', function () {
  ...
  let url =
    base_url +
    'admin/production/Indent/export_selected_frphp_excel?ids=' +
    selected.join(',');
